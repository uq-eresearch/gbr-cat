...
Ext.form.DateField = Ext.extend(Ext.form.TriggerField,  {
    ...
    setValue : function(date){
        ...
        if(!date) return date;
        if(!date.dateFormat){
           //look up the appropriate format method name to call
           var method = Date.formatFunctions[this.format];
           //create one if it doesn't exist
           if(!method){
              Date.createNewFormat(this.format);
              //try again
              method = Date.formatFunctions[this.format];
           }
           return Ext.isDate(date) ? Date.prototype[method].apply(date,[this.format]) : date;
        }
